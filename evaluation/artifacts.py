...
def write_per_well(out_dir: Path, per_well_df: pd.DataFrame) -> Path:
    ...
    per_well_df.to_csv(path, index=False, encoding="utf-8-sig")
    return path
...
def write_predictions_csv(
    ...
    planned_step: np.ndarray | None = None,        # (n, K) float, NaN on excluded rows
    exclude: np.ndarray | None = None,             # (n, K) bool
    exclude_reason: np.ndarray | None = None,      # (n, K) object[str], "" on non-excluded
) -> Path:
    ...
    have_tuples = tuple_topk_labels is not None
    K_tuple = tuple_topk_labels.shape[2] if have_tuples else 0

    records = []
    have_alignment = (
        planned_step is not None and exclude is not None and exclude_reason is not None
    )
    for seq in range(n):
        for step in range(n_future):
            ...
            # Alignment annotations (§2b/§2d): planned-position counter +
            # exclude flag + reason. Surfaced in CSV so analysts can audit
            # which rows the summary metrics dropped.
            if have_alignment:
                ps = planned_step[seq, step]
                row["planned_step"]   = None if ps != ps else int(ps)   # NaN-safe
                row["exclude"]        = bool(exclude[seq, step])
                reason = exclude_reason[seq, step]
                row["exclude_reason"] = reason if reason else None
            for head in HIERARCHY:
                if head in true_labels:
                    row[f"true_{head}"] = true_labels[head][seq, step]
                    pred_top1 = pred_top1_labels[head][seq, step]
                    topk = pred_topk_labels[head][seq, step]
                    is_null = (pred_top1 == "" or pred_top1 is None)
                    row[f"pred_{head}_top1"] = "" if is_null else pred_top1
                    row[f"pred_{head}_top3"] = "" if is_null else "|".join(topk.tolist())
                    row[f"{head}_in_top3"] = (
                        False if is_null
                        else bool(true_labels[head][seq, step] in topk)
                    )
            row["true_duration_hours"] = round(float(true_duration[seq, step]), 4)
            pred_dur = float(pred_duration[seq, step])
            if pred_dur != pred_dur:    # NaN -> shifted-NULL row
                row["pred_duration_hours"] = None
                row["duration_abs_err"] = None
            else:
                row["pred_duration_hours"] = round(pred_dur, 4)
                row["duration_abs_err"] = round(
                    float(abs(pred_dur - true_duration[seq, step])), 4
                )
            ...
            if "duration_bin" in true_labels:
                ...
                bin_is_null = (pred_bin_top1 == "" or pred_bin_top1 == "None")
                row["true_duration_bin"]      = true_bin
                row["pred_duration_bin_top1"] = "" if bin_is_null else pred_bin_top1
                row["pred_duration_bin_top3"] = (
                    "" if bin_is_null
                    else "|".join(str(x) for x in pred_bin_topk.tolist())
                )
                row["duration_bin_in_top3"] = (
                    False if bin_is_null
                    else bool(true_bin in [str(x) for x in pred_bin_topk])
                )
                if bin_centers is not None:
                    if bin_is_null:
                        row["duration_bin_top1_center_hours"] = None
            ...
            if have_tuples:
                ...
                # Detect shifted-NULL tuple row (all label slots empty for tuple 0).
                tuple_is_null = (
                    K_tuple == 0
                    or all(
                        (tuple_topk_labels[seq, step, 0, j] == "" or tuple_topk_labels[seq, step, 0, j] is None)
                        for j in range(min(4, tuple_topk_labels.shape[-1]))
                    )
                )
                ...
                for i in range(K_tuple):
                    ...
                    for j, h in enumerate(HIERARCHY):
                        row[f"pred_tuple_{i}_{h}"] = "" if tuple_is_null else tuple_topk_labels[seq, step, i, j]
                    if include_bins_in_tuples:
                        # 5D mode: 5th column of the joint tuple is the bin label.
                        row[f"pred_tuple_{i}_duration_bin"] = (
                            "" if tuple_is_null else tuple_topk_labels[seq, step, i, 4]
                        )
                    if tuple_topk_prob is not None:
                        prob = float(tuple_topk_prob[seq, step, i])
                        row[f"pred_tuple_{i}_prob"] = (
                            None if (tuple_is_null or prob != prob) else round(prob, 4)
                        )
                    ...
                    if tuple_topk_prob is not None:
                        prob = float(tuple_topk_prob[seq, step, i])
                        row[f"pred_{i}_state_prob"] = (
                            None if (tuple_is_null or prob != prob) else round(prob, 4)
                        )
                    if true_state is not None:
                        row[f"pred_{i}_state_match"] = (
                            False if tuple_is_null else (pred_state == true_state)
                        )
                ...
                if len(true_tup) == 4:
                    if tuple_is_null:
                        row["tuple_in_topk"] = False
                    else:
                        tuples_at_step = tuple_topk_labels[seq, step]   # (top_k, n_joint)
                        row["tuple_in_topk"] = bool(
                            any(tuple(tuples_at_step[i, :4]) == true_tup for i in range(K_tuple))
                        )
